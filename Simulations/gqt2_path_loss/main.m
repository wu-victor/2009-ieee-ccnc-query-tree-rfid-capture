...
while (~done_sing)
    ...
    if (length(match_indices) == 0) % No matches.  White leaf.
        ...
        if (length(match_indices) == 1) % SIR will be effectively Inf.  Check that prefix again next time (i.e. capture effect).
            black_count = black_count + 1;
            acked_indices(match_indices(1),1) = 1;
            
            height = length(Q_lens);
            Q(height+1, :) = -ones(1,k);
            Q(height+1,1:length(prefix)) = prefix;
            Q_lens(height+1,1) = length(prefix);
        ...
        if (length(match_indices) > 1) % Check SIRs.
            ...
            if (length(prefix) >= k)
                display('Error with prefix length.');
            end
            ...
            if (max_SIR(1) >= SIR_req) % Capture effect.  Check that prefix again next time.
                black_count = black_count + 1;
            ...
            else % No capture effect.  Just collision.  Grow tree.
